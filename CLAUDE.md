...
- `/review-paper` — Paper review & polish checklist: 14-item universal checklist + project-specific items. Items carry **stable slugs and never renumber** — an inapplicable item is marked `N/A` and keeps its number, so delegation by identity cannot fire on the wrong subject. Item 12 delegates to `/litrev`, item 13 to `/numerics-audit`, item 14 to `/ai-slop`. `checklist-only` mode builds the checklist and stops, for hosts that own their own ordering.
...
- `/ai-slop` — Multi-agent AI-slop and revision-language sweep: chunked parallel agents, Tier 1/2 triage, complements `/review-paper` item 14 for revision cycles
...
| `guides/paper-review-checklist.md` | 14-item paper polish checklist with canonical slugs (proofs, notation, numerics audit, style, bibliography) |
