...
#
# Recover image x* from blurred noisy observation b = A*x* + noise where A
# is a convolution (blur) operator. Reformulated as an NCP:
#   ψ(z) = min{z, H*z + c},  z >= 0
...
2D Gaussian blur kernel. `ksize` defaults to 2*ceil(3*sigma)+1 (odd).
"""
...
Generate a k × k test image in [0, 1]. Patterns: :blocks, :checkerboard, :gradient.
"""
...
function image_restoration_problem(x_true::Matrix{Float64};
    ...
    prob = TestProblem(0, prob_name, F_ncp, proj_nn,
                       "Image restoration NCP (GPSR)")
...
Image reconstruction quality metrics (assuming pixel range [0, 1]).
"""
